	...
	return (
		<div className="space-y-8 animate-pop-in">
			...
			<form onSubmit={handleSubmit} className="flex gap-4">
				<input
					className="flex-1 border-2 border-neo-black bg-white rounded-xl px-4 py-3 text-lg font-bold shadow-[2px_2px_0px_#1a1510] focus:ring-4 focus:ring-neo-orange focus:outline-none transition-all"
			...
			{!loading && decks.length === 0 && cards.length === 0 && initialQ && (
				<div className="bg-white border-2 border-neo-black rounded-2xl shadow-[2px_2px_0px_#1a1510] p-8 text-center mt-8">
			...
			{!loading && decks.length > 0 && (
				<section className="space-y-4 pt-4">
					...
					<div className="grid gap-6 md:grid-cols-2">
						{decks.map((deck) => (
							<UICard key={deck.id} className="bg-neo-yellow/20 border-2 border-neo-black rounded-2xl shadow-[2px_2px_0px_#1a1510] transition-transform hover:-translate-y-1 hover:shadow-[2px_2px_0px_#1a1510] p-4">
			...
			{!loading && cards.length > 0 && (
				<section className="space-y-4 pt-4">
					...
					<div className="grid gap-6 md:grid-cols-2">
						{cards.map((card) => (
							<UICard key={card.id} className="bg-white border-2 border-neo-black rounded-2xl shadow-[2px_2px_0px_#1a1510] p-4">
								<CardHeader className="pb-2">
